...
class Bp8ReleaseOperationsTest(unittest.TestCase):
    ...
    def build_release(self, version="0.8.0-test"):
        ...
        manifest = release_contract.build_manifest_from_layout(
            ...
            world_revision=1,
            generation_pack_version=version,
    ...
    def test_one_manifest_drives_client_and_server_and_repairs_drift(self):
        ...
        (server_root / "persistent" / "world").mkdir(parents=True)
        (server_root / "persistent" / "world" / "level.dat").write_bytes(b"world-state")
    ...
    def test_backup_checksum_and_clean_offhost_restore(self):
        manifest, _, _ = self.build_release()
        server_root = self.tmp / "server"
        world = server_root / "persistent" / "world"
        world.mkdir(parents=True)
        (world / "level.dat").write_bytes(b"abc123")
        ...
        self.assertEqual(
            release_contract.sha256_file(archive),
            archive.with_suffix(archive.suffix + ".sha256").read_text("ascii").strip(),
        )
    ...
    def test_failed_application_health_rolls_back_only_release_pointer(self):
        ...
        (root / "persistent" / "world").mkdir(parents=True)
        (root / "persistent" / "world" / "level.dat").write_text("irreversible-world\n")
        ...
        self.assertEqual(
            "irreversible-world\n",
            (root / "persistent" / "world" / "level.dat").read_text(),
        )
    ...
    def test_offline_structure_index_uses_center_nearest_interior_candidate(self):
        plan = {
            ...
            "strategicHerds": [{
                ...
                "species": "minecraft:cow",
                "origin": {"x": 0, "z": 0},
                "destination": {"x": 2048, "z": 0},
                "count": 80,
                "speedBlocksPerSecond": 1.25,
            }],
        }
        mappings = {"sources": [{
            "structureId": "minecraft:pillager_outpost",
            "sourceClass": "CAMP",
            "factionId": "drewcraft:raiders",
        }]}
        index = {"structures": [{
            "dimension": "minecraft:overworld",
            "structureId": "minecraft:pillager_outpost",
